...
parser.add_argument("--config", type=str, default="D:/WorkspaceForC/U2PL/finaldata/config.yaml")
parser.add_argument("--local_rank", type=int, default=0)
parser.add_argument("--seed", type=int, default=0)
parser.add_argument("--port", default=None, type=int)
parser.add_argument("--rand_resize", default=True)
parser.add_argument("--rand_rotation", default=True)
parser.add_argument("--flip", default=True)
parser.add_argument("--GaussianBlur", default=True)
...
def main():
    ...
    if cfg["saver"].get("auto_resume", False):
        ...
        lastest_model = os.path.join(cfg["save_path"], "final.pth")
        lastest_model = "D:/WorkspaceForC/U2PL/finaldata/checkpoints/final.pth"
        ...
        if not os.path.exists(lastest_model):
            ...
            best_prec, _ = load_state(
                lastest_model, model, optimizer=optimizer, key="teacher_state"
            )
            _, _ = load_state(
    ...
    # checkpoint = torch.load(cfg["saver"]["pretrain"])
    # #print(checkpoint.keys())
    # saved_state_dict = convert_state_dict(checkpoint)
    # model.load_state_dict(saved_state_dict, strict=False)
    # model.load_state_dict(saved_state_dict, strict=False)
    # model_teacher.load_state_dict(saved_state_dict, strict=False)
       ...
       #Training
        ...
        if cfg_trainer["eval_on"]:
            ...
            if prec > best_prec:
                best_prec = prec
                torch.save(
                    state, osp.join(cfg["saver"]["snapshot_dir"], "final.pth")
                )

            torch.save(state, osp.join(cfg["saver"]["snapshot_dir"], "final.pth"))
...
def train(
    ...
    for step in range(100):
        ...
        #label_l = np.where(label_l == 1, 14, label_l)
        #label_l = np.where(label_l == 4, 10, label_l)
        #label_l = np.where(label_l == 2, 15, label_l)
        #label_l = np.where(label_l == 3, 16, label_l)
        #label_l = np.where(label_l == 0, 100, label_l)
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 1):
            ...
            if epoch == cfg["trainer"].get("sup_only_epoch", 100):
...
def validate(
    ...
    for step, batch in enumerate(data_loader):
        ...
        #color_path = os.path.join("checkpoints", str(cnt) + ".png")
