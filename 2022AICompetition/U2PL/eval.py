...
def get_parser():
    ...
    parser.add_argument(
        "--base_size", type=int, default=1464, help="based size for scaling"
    )
    parser.add_argument(
        "--scales", type=float, default=[0.5], nargs="+", help="evaluation scales"
    )
    parser.add_argument("--config", type=str, default="finaldata/config.yaml")
    parser.add_argument(
        "--model_path",
        type=str,
        default="finaldata/checkpoints/ckpt_best.pth",
...
def valiadte_whole(
    ...
    for i, (input_pth, _) in enumerate(data_list):
        ...
        image_path, _ = data_list[i]
        image_name = image_path.split("/")[-1].split(".")[0]
        image_name = image_name.replace("labeled_images\\", "")
