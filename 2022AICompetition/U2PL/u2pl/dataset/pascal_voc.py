...
class voc_dset(BaseDataset):
    def __init__(
        self, data_root, data_list, trs_form, seed=0, n_sup=100, split="val"
    ):
...
def build_transfrom(cfg):
    ...
    if cfg.get("resize", False):
        trs_form.append(psp_trsform.Resize(cfg["train"]["resize"]))
    if cfg.get("rand_resize", True):
        trs_form.append(psp_trsform.RandResize(cfg["train"]["rand_resize"]))
    if cfg.get("rand_rotation", False):
        rand_rotation = cfg["train"]["rand_rotation"]
        trs_form.append(
            psp_trsform.RandRotate(rand_rotation, ignore_label=ignore_label)
        )
    if cfg.get("GaussianBlur", True) and cfg["train"]["GaussianBlur"]:
        trs_form.append(psp_trsform.RandomGaussianBlur())
    if cfg.get("flip", True) and cfg.get("flip"):
